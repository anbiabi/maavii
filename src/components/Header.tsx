...
interface HeaderProps {
  onCartToggle: () => void;
  currentPage?: 'home' | 'ordering' | 'grocery' | 'education' | 'menu' | 'standaloneMenu';
  onPageChange?: (page: 'home' | 'ordering' | 'grocery' | 'education' | 'menu' | 'standaloneMenu') => void;
}

const Header: React.FC<HeaderProps> = ({ onCartToggle, currentPage = 'home', onPageChange }) => {
  const { language, setLanguage, t } = useLanguage();
  const { itemCount } = useCart();
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const navLinks = [
    { name: t('navHome'), href: '#home', action: 'home' },
    { name: t('navAbout'), href: '#about', action: 'home' },
    { name: t('navServices'), href: '#services', action: 'home' },
    { name: t('navMenu'), href: '#menu', action: 'menu' },
    { name: 'Grocery', href: '/grocery', action: 'grocery', isBadge: false },
    { name: 'Education', href: '/education', action: 'education', isBadge: false },
    { name: 'Services', href: '#services', action: 'home', isBadge: false },
    { name: t('navContact'), href: '#contact', action: 'home' },
  ];

  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string, action?: string) => {
    e.preventDefault();

    // Handle external links or hash links differently
    if (href.startsWith('/')) {
      // This is a route change, use onPageChange
      if (action && onPageChange) {
        setIsMenuOpen(false);
        onPageChange(action as 'home' | 'ordering' | 'grocery' | 'education' | 'menu' | 'menuItems' | 'groceryItems' | 'educationItems');
        window.scrollTo({ top: 0, behavior: 'smooth' });
      }
    } else if (href.startsWith('#')) {
      // This is a hash link, handle scrolling
      ...
      // If this hash link should change the page context, update it
      if (action && action !== 'home' && onPageChange) {
        onPageChange(action as 'home' | 'ordering' | 'grocery' | 'education' | 'menu' | 'menuItems' | 'groceryItems' | 'educationItems');
      }
    }
  };
        ...
        <nav className="hidden xl:flex items-center gap-7">
          {navLinks.map(link => (
            <a
              key={link.name}
              href={link.href}
              onClick={(e) => handleNavClick(e, link.href, link.action)}
              className={`relative py-2 text-sm font-black uppercase tracking-widest transition-colors duration-300 group/link ${
                ((link.action && link.action === currentPage) || (link.href.startsWith('#') && window.location.hash === link.href)) || link.isBadge ? 'text-maaviiOrange' : 'text-charcoal hover:text-maaviiOrange'
              }`}
            >
              {link.name}
